...
/**
 * Resolves and loads the openFDA Drugs@FDA JSON dataset, whether it
 * comes from a local file, a local zip, or a fresh download.
 */
class OpenFdaDrugsFdaDataset
{
    public const DEFAULT_URL = 'https://download.open.fda.gov/drug/drugsfda/drug-drugsfda-0001-of-0001.json.zip';
    ...
    public function resolveJsonPath(?string $path, bool $freshDownload, string $storageRelative = 'openfda'): string
    {
        ...
        $zipPath = $storageDir.'/drug-drugsfda.json.zip';

        return Cache::lock('openfda:drugsfda:download', 600)->block(600, function () use ($freshDownload, $zipPath, $storageDir): string {
    ...
    public function loadResults(string $jsonPath): array
    {
        ...
        if (! is_array($decoded) || ! isset($decoded['results']) || ! is_array($decoded['results'])) {
            throw new RuntimeException("Invalid openFDA Drugs@FDA dataset JSON: {$jsonPath}");
        }
    ...
    private function extractJsonFromZip(string $zipPath, string $destinationDir): string
    {
        ...
        $targetPath = rtrim($destinationDir, '/').'/drug-drugsfda.json';
