...
class EpcisSchemaVersionTest extends TestCase
{
    ...
    public function tenant_can_opt_out_of_accept_20_when_platform_flag_on(): void
    {
        ...
        $tenant = \App\Models\Tenant::query()->find('13fe9068-cb05-4bab-9e0e-a89f2a458832');
        if ($tenant === null) {
        ...
        try {
            \App\Support\TenantSettings::forTenant($tenant)->setEpcisAccept20(false);
            $tenant->save();
            ...
            \App\Support\TenantSettings::forTenant($tenant)->setEpcisAccept20(null);
            $tenant->save();
            tenancy()->end();
        }
    }
