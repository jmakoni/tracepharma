...
return new class extends Migration
{
    public function up(): void
    {
        ...
        if (! Schema::hasColumn('catalog_trading_partners', 'sgln')) {
            DB::statement("ALTER TABLE catalog_trading_partners ADD COLUMN sgln VARCHAR(50) GENERATED ALWAYS AS (IF(gln IS NULL OR CHAR_LENGTH(gln) < 12, NULL, CONCAT('urn:epc:id:sgln:', LEFT(gln, 12), '.0'))) STORED");
        }
    }
    ...
    {
        if (Schema::hasColumn('catalog_trading_partners', 'sgln')) {
            DB::statement('ALTER TABLE catalog_trading_partners DROP COLUMN sgln');
        }
